...
def res18unet(src_ch, tar_ch, pretrained=False, instance=True, **kwargs):
    """Constructs a ResUNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResUNet(BasicBlock, [2, 2, 2, 2],
...
def res34unet(src_ch, tar_ch, pretrained=False, instance=True, **kwargs):
    """Constructs a ResUNet-34 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResUNet(BasicBlock, [3, 4, 6, 3],
                    src_ch, tar_ch, instance=instance)
    if pretrained:
        from collections import OrderedDict
        pretrained_state = model_zoo.load_url(model_urls['resnet18'])
...
if __name__ == "__main__":
    ...
    for inst in [True, False]:
        generator = res18unet(args.src_ch, args.tar_ch, True, inst)
        gen_y = generator(x)
        ...
        generator = res34unet(args.src_ch, args.tar_ch, True, inst)
        gen_y = generator(x)
        total_params = sum(p.numel() for p in generator.parameters())
