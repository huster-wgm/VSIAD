...
class ResYNetSyncNB(nn.Module):

    def __init__(self, block, layers, src_ch=1, tar_ch=[1, 2]):
        super(ResYNetSyncNB, self).__init__()
        ...
        self.layer1 = self._make_layer(block, kernels[0], layers[0], stride=1)
        ...
        # Left arm
        self.upconvL10 = upconv(kernels[3], kernels[2], ratio="x2")
        self.inplanes = kernels[2]
        self.upResL1 = self._make_layer(block, kernels[2], layers[2])
        self.upconvL11 = upconv(kernels[2], kernels[1], ratio="x2")
        self.inplanes = kernels[1]
        self.upResL2 = self._make_layer(block, kernels[1], layers[1])
        self.upconvL12 = upconv(kernels[1], kernels[0], ratio="x2")
        self.inplanes = kernels[0]
        self.upResL3 = self._make_layer(block, kernels[0], layers[0])
        self.upconvL13 = upconv(kernels[0],  kernels[0], ratio="x2")
        self.predL = nn.Conv2d(kernels[0], tar_ch[0], kernel_size=3, stride=1, padding=1, bias=False)

        # Right arm
        self.upconvR10 = upconv(kernels[3], kernels[2], ratio="x2")
        self.inplanes = kernels[2]
        self.upResR1 = self._make_layer(block, kernels[2], layers[2])
        self.upconvR11 = upconv(kernels[2], kernels[1], ratio="x2")
        self.inplanes = kernels[1]
        self.upResR2 = self._make_layer(block, kernels[1], layers[1])
        self.upconvR12 = upconv(kernels[1], kernels[0], ratio="x2")
        self.inplanes = kernels[0]
        self.upResR3 = self._make_layer(block, kernels[0], layers[0])
        self.upconvR13 = upconv(kernels[0], kernels[0], ratio="x2")
...
def res18ynetsyncNB(src_ch, tar_ch, pretrained=False, instance=True, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResYNetSyncNB(BasicBlock, [2, 2, 2, 2],
                          src_ch, tar_ch)
    if pretrained:
...
def res34ynetsyncNB(src_ch, tar_ch, pretrained=False, instance=True, **kwargs):
    """Constructs a ResNet-34 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResYNetSyncNB(BasicBlock, [3, 4, 6, 3],
                          src_ch, tar_ch)
    if pretrained:
        from collections import OrderedDict
        pretrained_state = model_zoo.load_url(model_urls['resnet18'])
...
if __name__ == "__main__":
    ...
    for inst in [True, False]:
        generator = res18ynetsyncNB(args.src_ch, [args.tar1_ch, args.tar2_ch], True)
        gen_L, gen_R = generator(x)
        total_params = sum(p.numel() for p in generator.parameters())
        print("res18ynetsyncNB : {}".format("inst =>" if inst else " =>"))
        print(" Network L-output : ", gen_L.shape)
        print(" Network R-output : ", gen_R.shape)
        print(" Params: {:0.1f}M".format(total_params / (10**6)))

        generator = res34ynetsyncNB(args.src_ch, [args.tar1_ch, args.tar2_ch], True)
        gen_L, gen_R = generator(x)
        total_params = sum(p.numel() for p in generator.parameters())
        print("res34ynetsyncNB: {}".format("inst =>" if inst else " =>"))
        print(" Network L-output : ", gen_L.shape)
        print(" Network R-output : ", gen_R.shape)
        print(" Params: {:0.1f}M".format(total_params / (10**6)))
