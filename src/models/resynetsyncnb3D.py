...
class ResYNetSyncNB3D(nn.Module):

    def __init__(self, block, layers, src_ch=1, tar_ch=[1, 2]):
        super(ResYNetSyncNB3D, self).__init__()
...
def res18ynetsyncNB3D(src_ch, tar_ch, pretrained=False, instance=True, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResYNetSyncNB3D(BasicBlock3D, [2, 2, 2, 2],
                            src_ch, tar_ch)
    # if pretrained:
...
def res34ynetsyncNB3D(src_ch, tar_ch, pretrained=False, instance=True, **kwargs):
    """Constructs a ResNet-34 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResYNetSyncNB3D(BasicBlock3D, [3, 4, 6, 3],
                            src_ch, tar_ch)
    # if pretrained:
    #     from collections import OrderedDict
    #     pretrained_state = model_zoo.load_url(model_urls['resnet18'])
...
if __name__ == "__main__":
    ...
    for inst in [True, False]:
        generator = res18ynetsyncNB3D(args.src_ch, [args.tar1_ch, args.tar2_ch], True, inst)
        gen_L, gen_R = generator(x)
        total_params = sum(p.numel() for p in generator.parameters())
        print("res18ynetsyncNB3D : {}".format("inst =>" if inst else " =>"))
        print(" Network L-output : ", gen_L.shape)
        print(" Network R-output : ", gen_R.shape)
        print(" Params: {:0.1f}M".format(total_params / (10**6)))

        generator = res34ynetsyncNB3D(args.src_ch, [args.tar1_ch, args.tar2_ch], True, inst)
        gen_L, gen_R = generator(x)
        total_params = sum(p.numel() for p in generator.parameters())
        print("res34ynetsyncNB3D: {}".format("inst =>" if inst else " =>"))
        print(" Network L-output : ", gen_L.shape)
        print(" Network R-output : ", gen_R.shape)
        print(" Params: {:0.1f}M".format(total_params / (10**6)))
